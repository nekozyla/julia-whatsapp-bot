    ...
    try {
        ...
        for (const file of files) {
            if (file.endsWith('.json')) {
                ...
                if (sessions[sessionKeyFromFile]) continue; 
                const loadedHistory = await loadSessionHistory(sessionKeyFromFile);
                if (loadedHistory?.length > 0) {
                    sessions[sessionKeyFromFile] = model.startChat({
                        history: loadedHistory,
                        generationConfig: { maxOutputTokens: DEFAULT_MAX_OUTPUT_TOKENS },
                    });
                    console.log(`[Persistência] Sessão carregada na inicialização para ${sessionKeyFromFile}`);
                }
            }
        }
...
module.exports = { saveSessionHistory, getOrCreateChatForSession, loadAllPersistedSessions, clearSession };
